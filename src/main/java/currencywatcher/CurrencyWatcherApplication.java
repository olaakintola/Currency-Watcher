...
import currencywatcher.email.EmailNotification;
...
/*		List<CurrencyEntity> priceData = new ArrayList<CurrencyEntity>();
	...
	public void eachValue() throws IOException {
		List<CurrencyEntity> lst = currencyEntityRepository.findAll();
		...
		for (CurrencyEntity currencyEntity : lst) {
			for (UserDetails userDetails : auser) {
				log.debug(userDetails.getCurrenciesType());

				if(shouldMailBeSent(userDetails, currencyEntity)) {
					System.out.println("Send Email for " + userDetails.getAboveBelow() + " currency: " + userDetails.getCurrenciesType());
					tempUser = userDetails;
					sendMail(emailSent, tempUser);
				}
			}
		}
	...
	private boolean shouldMailBeSent(UserDetails userDetails, CurrencyEntity currencyEntity) {
		double currentValue = currencyEntity.grabValue(userDetails.getCurrenciesType(), userDetails.getCurrencyChoice());
		if(userDetails.isAbove() && currentValue > userDetails.getPrice()) {
			return true;
		}

		if(userDetails.isBelow() && currentValue < userDetails.getPrice()) {
			return true;
		}
	...
	private void sendMail(String emailSent, UserDetails tempUser) {
